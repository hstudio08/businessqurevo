...
export default function LoginPage() {
  ...
  const handleSendOTP = async (e: React.FormEvent) => {
    ...
    if (!existingUsers.includes(email) && email !== "admin@qurevo.in") {
  ...
  const handleVerifyOTP = (e: React.FormEvent) => {
    ...
    if (userOtpInput === generatedOtp) {
      ...
      localStorage.setItem('qurevo_session', email);
      
      if (email === "admin@qurevo.in") {
        router.push('/admin');
      } else {
        ...
        <AnimatePresence mode="wait">
          {!otpSent ? (
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input 
                    ...
                    className="w-full bg-black/40 border border-white/10 rounded-xl pl-10 pr-4 py-3 text-white focus:outline-none focus:border-lime_cream/50"
            ...
            <motion.form key="otp-form" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onSubmit={handleVerifyOTP} className="space-y-6">
              ...
              <button type="submit" className="w-full bg-[#ECF39E] text-[#132A13] hover:bg-[#d8e08d] py-3.5 rounded-xl font-bold transition-all shadow-lg flex items-center justify-center gap-2">
                Verify & Login <ArrowRight size={18} />
              </button>
              <button type="button" onClick={() => setOtpSent(false)} className="w-full text-sm text-gray-400 hover:text-white flex items-center justify-center gap-1 transition-colors">
